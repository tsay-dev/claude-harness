...
| 渡されるもの | 使い方 |
| --- | --- |
| `videos/<format>/<id>/brief.yaml` | 企画。**台本に載せてよい事実はここの `points:` / `sources:` だけ** |
| `channel/identity.md` | 誰に向けるか・人格・**禁止表現** |
| `channel/voice.md` | 口調・語尾・**話速（文字/秒）** |

## 🚫 事実を発明しない

`points:` に無いことを書いてはならない。調べるな。推測で埋めるな。
...
`points:` に根拠が無ければ書いてはならない。台本らしく響く一般命題ほど、根拠なく紛れ込みやすい。
書きたくなったら、それが `points:` のどれから導けるかを確かめ、導けないなら `⚠要確認` にする。
