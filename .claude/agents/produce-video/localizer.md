...
書式の SSOT は [schema.md](../../rules/produce-video/schema.md)（`i18n/<lang>.json` の節）。本文に複製しない。
...
4. `i18n/<lang>.json` を書き、**自分で機械検査を叩く**:
   ```bash
   python3 .claude/tools/produce-video/produce-video.py check <video_dir> --stage i18n
   ```
   ERROR は自分で直してから返す。
