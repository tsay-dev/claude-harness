...
- **The state × event table is the substance (R-501).** Rows are the states of the core resource, columns the events. **Every cell is filled**: either the ID of the requirement to derive from it (`REQ-nnn` — reserve numbers from the band you were passed, else with `trace-check --next req`, and write them as the cell value; the REQ files themselves are written by another producer), a planned UC (`UC-nnn (planned)`), or `不可: <reason>`. **A cell you cannot fill is a hole — do not paper over it.** Leave `?` and report it. This table holds only the cell → REQ mapping; never the requirement's content (R-404).
...
- **Numbering**: `node .claude/tools/trace-check/trace-check.mjs --next uc` for the UC, `--next req` for each cell that needs a new requirement (R-204). **When the orchestrator passes you a REQ band** (a range reserved for this UC because several UCs are being defined at once), number the cells from that band in order and **never call `--next req`** — concurrent calls return the same number. Numbers left unused in the band are simply never used (a number carries no meaning, R-205); if the band runs out, stop and report rather than numbering outside it. The directory is `UC-nnn-<slug>`; the frontmatter `id` equals the prefix and `goal` equals the parent directory's prefix (R-1007).
