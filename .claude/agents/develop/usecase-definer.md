...
- **Numbering**: `node .claude/tools/trace-check/trace-check.mjs --next uc` for the UC, and `--next req --reserve N` once for the N cells that need a new requirement (R-204; one call per cell is fine too). The tool reserves what it returns, so other producers running at the same time cannot receive your numbers — never derive a number by eye or by arithmetic on an earlier result. The directory is `UC-nnn-<slug>`; the frontmatter `id` equals the prefix and `goal` equals the parent directory's prefix (R-1007).
