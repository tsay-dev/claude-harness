...
- **Partition and assertion (the residue the machine cannot see)**: `trace-check` C10 / C11 prove that the classes in each REQ's `## 検証方針` and the tests agree; they cannot tell whether the classes exhaust the input space, or whether each test's assertion verifies the meaning of its sentence rather than merely exercising the code. Judge both. A unit that realizes a REQ / BR with no `@implements` is a defect too.
