...
2. **Use cases** — launch `usecase-definer` **one Task per UC, concurrently across UCs** (🙋 per UC). **Input**: the goal's `GOAL.md`, `02-actors.md`, `01-glossary.md`, one neighboring UC directory in the same goal if any, the existing BRs, as paths — and, **when launching several UC Tasks at once, a REQ band per Task**: run `trace-check --next req` once (it counts IDs already reserved in UC tables) and hand each Task a disjoint range in launch order (20–30 numbers each; gaps are fine, R-205). Concurrent Tasks never call `--next req` themselves — they would all receive the same number. **Done when**: the state × event table has no empty cell and every cell names a reserved REQ ID, a planned UC, or `不可` with a reason; the exception sweep covers the 4 axes. On approval, `status: active`.
