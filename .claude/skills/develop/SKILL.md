...
- **Launch order**:
  ...
  5. Once FE and BE implementations are both in, advance `phase:` to `検証` and launch **`slice-reviewer`** (🔴; Cursor model: the fable family — §5 / ADR-0020). Iterate to zero defects (round limit in §4). **`slice-attacker` / `system-attacker` are not launched in this phase.**
...
**In Cursor, choose the model per Task at each launch.** A projected agent's `model:` is normalized to `inherit`, so do not leave it entirely to the parent's default. Choose from the candidates the Task's `model` argument offers at that launch (never invent a slug). When the human names a model, follow it.

- **Judgment zone** (the SSOT / DB / contract / test design / ADR / oracle / reviewer / attacker): pick whichever offered candidate degrades least in judgment quality. Inherit is forbidden.
- **`slice-reviewer` (stricter)**: inherit is forbidden, and a cheaper model is forbidden even to save cost. From the candidates offered at that launch, pick the one whose family the runtime names **fable** (the strongest-reasoning / thinking candidate); when several versions of that family are offered, pick the latest. If that family is not listed, do not silently fall back — tell the human which models were available and stop, unless the human names a substitute. A false defect list from a weaker model becomes a wrong implementation; an empty list from a weaker model is a false green of the definition of done (ADR-0020).
- **Deterministic zone** (the 3 implementers / skeleton / committer): lighter, or `inherit`, is fine.

Do not hardcode a versioned catalog id (a Cursor kebab-case slug, and the like) into the harness. A family name used as a launch-time selection criterion (`fable`) is not that pin.
...
| `slice-reviewer` | P4, after implementation | the slice, the UC directory, BE tests, change scope (on re-verification: previous defects + change scope) | 🔴 independent judgment (Cursor: fable family) |
